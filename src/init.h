...
inline unsigned naive_init_side(std::vector<Cell *> &cell_map) {
...
inline unsigned sophisticated_init_side(std::vector<Net *> &net_map,
    ...
    vector<Net *> net_list(net_map.begin(), net_map.end());
...
inline unsigned init_side(std::vector<Net *> &net_map,
                          std::vector<Cell *> &cell_map,
...
inline void gains(std::vector<Net *> &net_map, std::vector<Cell *> &cell_map,
                  const bool checking) {
    using namespace std;
        ...
        if (cnt == 0 || cnt == cells.size()) {
            ...
            if (cnt == 1) {
                for (idx = count = 0; idx < cells.size(); ++idx) {
                    const unsigned cell_name(cells[idx]);
                    Cell *cell = cell_map[cell_name];
                    if (cell->getSide()) ++(simulation[cell_name]), ++count;
                }
...
inline void init_gains(std::vector<Net *> &net_map,
                       std::vector<Cell *> &cell_map) {
    gains(net_map, cell_map, false);
}

inline void assert_gains(std::vector<Net *> &net_map,
                         std::vector<Cell *> &cell_map) {
